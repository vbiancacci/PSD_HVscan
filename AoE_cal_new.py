...
class standard_aoe_with_high_tail(PDF):
    ...
    def bounds(self, guess, n_sig, n_bkg,**kwargs):
    #def bounds(self, n_sig, n_bkg, **kwargs):
        
        bounds_dict = {"n_sig": n_sig, #(0,2*np.sum(hist)),
                      "mu": (None,None),
                      "sigma": (0,None),
                      "htail": (0,1),
                      "tau_sig": (None,0),
                      "n_bkg": n_bkg, #0,2*np.sum(hist)),
                      "tau_bkg":(0,None),
                      "lower_range":(None,None),
                      "upper_range":(None,None),
                      "components":(None,None)}
...
def unbinned_aoe_fit(
    ...
    if display > 1:
        ...
        return m.values, m.errors, m.migrad, m.covariance
